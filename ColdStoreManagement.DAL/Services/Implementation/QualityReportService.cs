...
namespace ColdStoreManagement.DAL.Services.Implementation
{
    public class QualityReportService : IQualityReportService
    {
        ...
        public QualityReportService(SQLHelperCore sqlHelper)
        {
        ...
        {
            // Reusing generateinwardReport logic as seen in research
            List<SqlParameter> parameters = new List<SqlParameter>
            {
                ...
                new SqlParameter("@Chambername", request.ChamberName ?? (object)DBNull.Value),
                new SqlParameter("@chamberid", 0)
            };

            await _sqlHelper.ExecuteNonQueryAsync(CommandType.StoredProcedure, "generateinwardReport", parameters.ToArray());
